...
class ApplicationMain {
	...
	public static function main () {
		...
		//nme.Lib.setPackage("VitaL", "Lurker", "dru.Lurker", "1.0.0");
	...
	private static function start ():Void {
		...
		var hasMain = false;
		var mainClass = Type.resolveClass ("dru.Main");
...
class ApplicationMain {
	...
	public static function main () {
		...
		for (methodName in Type.getClassFields (dru.Main)) {
		...
		if (hasMain) {
			...
			Reflect.callMethod (dru.Main, Reflect.field (dru.Main, "main"), []);
...
@:keep class DocumentClass extends dru.Main { }
